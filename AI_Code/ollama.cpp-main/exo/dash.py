...
class PackageSizeTracker:
    ...
    def generate_report(self, data: List[Dict], output_dir: str = "reports") -> Optional[str]:
        ...
        size_figure_json = fig_size.to_json() if fig_size.data else 'null'
        lines_figure_json = fig_lines.to_json() if fig_lines.data else 'null'
        tokens_figure_json = fig_tokens.to_json() if fig_tokens.data else 'null'
        ...
        <body>
            ...
            <script>
                // Store figure data
                const sizeFigure = {size_figure_json};
                const linesFigure = {lines_figure_json};
                const tokensFigure = {tokens_figure_json};
                
                let globalMinDate = null;
                let globalMaxDate = null;

                function initializeCharts() {{
                    // Initialize each chart with its own figure data
                    if (sizeFigure && sizeFigure.data) {{
                        Plotly.newPlot('size-chart', sizeFigure.data, sizeFigure.layout);
                    }}
                    if (linesFigure && linesFigure.data) {{
                        Plotly.newPlot('lines-chart', linesFigure.data, linesFigure.layout);
                    }}
                    if (tokensFigure && tokensFigure.data) {{
                        Plotly.newPlot('tokens-chart', tokensFigure.data, tokensFigure.layout);
                    }}
                }}
                ...
                function updatePlotRange(startDate, endDate) {{
                    ...
                    if (sizeFigure && sizeFigure.data) {{
                        Plotly.relayout('size-chart', {{'xaxis.range': [startDate, endDate]}});
                    }}
                    if (linesFigure && linesFigure.data) {{
                        Plotly.relayout('lines-chart', {{'xaxis.range': [startDate, endDate]}});
                    }}
                    if (tokensFigure && tokensFigure.data) {{
                        Plotly.relayout('tokens-chart', {{'xaxis.range': [startDate, endDate]}});
                    }}
                }}
                ...
                function findDateRange() {{
                    ...
                    if (sizeFigure && sizeFigure.data && sizeFigure.data[0] && sizeFigure.data[0].x) {{
                        dates.push(...sizeFigure.data[0].x);
                    }}
                    if (linesFigure && linesFigure.data && linesFigure.data[0] && linesFigure.data[0].x) {{
                        dates.push(...linesFigure.data[0].x);
                    }}
                    if (tokensFigure && tokensFigure.data && tokensFigure.data[0] && tokensFigure.data[0].x) {{
                        dates.push(...tokensFigure.data[0].x);
                    }}
                ...
                $(document).ready(function() {{
                    ...
                    const {{ minDate, maxDate }} = findDateRange();
