...
public:
    ...
    void selectDevice(int device_id) {
        if (device_id >= 0 && device_id < available_devices.size()) {
            ...
            // IMPORTANT: Using ONEAPI_DEVICE_SELECTOR instead of deprecated SYCL_DEVICE_FILTER
...
public:
    ...
    bool initialize(int device_id = 0) {
        ...
        // SYCL-specific optimizations for Intel GPUs
        // These are still relevant for performance on Intel Arc GPUs
    ...
    void benchmark(int iterations = 5) {
        ...
        double avg_time = 0;
        double avg_tokens = 0;
        double min_time = times;
        double max_time = times;
...
int main(int argc, char* argv) {
    try {
        if (argc < 2) {
            std::cout << "Usage: " << argv << " <model_path> [gpu_layers][device_id]" << std::endl;
            std::cout << "Example: " << argv << " models/llama-2-7b-chat.Q4_0.gguf 33 0" << std::endl;
        ...
        std::string model_path = argv[span_0](start_span)[span_0](end_span);
        int gpu_layers = argc > 2? std::atoi(argv[span_1](start_span)[span_1](end_span)) : 99;
        int device_id = argc > 3? std::atoi(argv[span_2](start_span)[span_2](end_span)) : 0;
